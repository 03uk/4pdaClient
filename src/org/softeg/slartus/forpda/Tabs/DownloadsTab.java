...
public class DownloadsTab extends BaseTab implements AdapterView.OnItemClickListener {
    ...
    private DownloadTasks getDownloadTasks() {
        DownloadTasks downloadTasks = Client.INSTANCE.getDownloadTasks();
    ...
    public DownloadsTab(Context context, String tabTag) {
        super(context);
        m_TabId = tabTag;
    ...
    private View createListFooter() {
        ...
        m_ListFooter.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                if (Client.INSTANCE.getDownloadTasks().getFullLength() > Client.INSTANCE.getDownloadTasks().size())
                    loadMore();
            }
        });
    ...
    private void updateDataInfo() {
        ...
        int loadMoreVisibility = (Client.INSTANCE.getDownloadTasks().getFullLength() > Client.INSTANCE.getDownloadTasks().size()) ? View.VISIBLE : View.GONE;
        txtPullToLoadMore.setVisibility(loadMoreVisibility);
        imgPullToLoadMore.setVisibility(loadMoreVisibility);
        txtLoadMoreThemes.setText("Всего: " + Client.INSTANCE.getDownloadTasks().getFullLength());

        m_ListFooter.setVisibility(Client.INSTANCE.getDownloadTasks().size() > 0 ? View.VISIBLE : View.GONE);
    }
